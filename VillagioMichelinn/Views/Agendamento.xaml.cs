...
namespace VillagioMichelinn
{
	public partial class Agendamento : ContentPage
	{
		...
		private async void OnPagarClicked(object sender, EventArgs e)
		{
			...
			string horario24 = horarioSelecionado.Trim();
			...
			int familiaId = Preferences.Get("FamiliaId", 0);

			if (familiaId <= 0)
			{
				await DisplayAlert("Erro", "Família não identificada. Faça login novamente.", "OK");
				return;
			}

			var requestApi = new VillagioMichelinn.Models.CriarReservaRequest
			{
				FamiliaId = familiaId,
				Data = dataIso,
				Horario = horario24
			...
			var api = new ReservaApiService();
			var (sucesso, mensagem) = await api.CriarReservaAsync(requestApi);

			if (!sucesso)
			{
				await DisplayAlert("Erro ao registrar reserva", mensagem, "OK");
				return;
			}
			...
			await DisplayAlert("Sucesso", "Sua reserva já foi registrada! Agora finalize o pagamento.", "OK");
